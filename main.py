...
import json
import requests
from apps import BananaText, SudokuGrid, Shell
...
class Main(Frame):
    def __init__(self, root, height, width, bg):
        ...
        self.apps = {'BananaText': [self.start_BananaText], 'Sudoku': [self.start_sudoku], 'Shell': [self.start_shell]}
        self.appbar = []
    ...
    def draw_app_bar(self):
        ...
        for appname, app in self.apps.items():
            ...
            appbar.append(Button(command=app[0], text=appname))
            appbar[x].grid(row=1, column=x+1)
            x += 1
